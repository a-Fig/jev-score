...
function renderWorkspace() {
  ...
  const questionOptions = active?.questions.map((question) => `<option value="${question.key}" ${state.question === question.key ? "selected" : ""}>${question.direction === "lower" ? "↓" : "↑"} ${escapeHtml(question.text)}</option>`).join("") || "";
  ...
  $("#delete-workspace").onclick = async () => {
    $("#workspace-menu").removeAttribute("open");
    if (confirm(`Delete “${workspace.name}” and every document and score inside it?`)) { await api(`/api/workspaces/${workspace.id}`, { method: "DELETE" }); state.workspace = null; state.group = null; state.question = ""; state.mode = null; state.compare = []; await loadDashboard(); state.dashboard.workspaces.length ? openWorkspace(state.dashboard.workspaces[0].id) : renderEmpty(); }
  };
...
function comparisonScoreRows(leftId, rightId) {
  ...
  const rows = [
    { label: "Overall", left: left?.overallScore, right: right?.overallScore, direction: "higher", overall: true },
    ...matrix.questions.map((question) => ({ label: question.text, left: left?.scores[question.key], right: right?.scores[question.key], direction: question.direction })),
  ];
  const cells = (row) => {
    const values = [row.left, row.right].filter(Number.isFinite);
    const best = values.length ? (row.direction === "lower" ? Math.min(...values) : Math.max(...values)) : null;
    return [row.left, row.right].map((value) => `<td class="${value != null && value === best ? "comparison-winner" : ""}">${score(value)}</td>`).join("");
  };
  return `<div class="comparison-score-table"><table><thead><tr><th>Metric</th><th title="${escapeHtml(left?.title || "Left document")}">${left ? versionLabel(left) : "Left"}</th><th title="${escapeHtml(right?.title || "Right document")}">${right ? versionLabel(right) : "Right"}</th></tr></thead><tbody>${rows.map((row) => `<tr class="${row.overall ? "overall-row" : ""}"><td>${escapeHtml(row.label)} <span class="direction ${row.direction}">${row.direction === "lower" ? "↓ lower" : "↑ higher"}</span></td>${cells(row)}</tr>`).join("")}</tbody></table></div>`;
}
...
function renderGroups() {
  ...
  state.workspace = null; state.compare = []; renderCompareTray(); loadDashboard().then(() => {
    main.innerHTML = `<section class="groups-page"><div class="topbar"><div><p class="eyebrow">Library</p><h1>Evaluation groups</h1><p class="subtle">Reusable questions that can be attached to any workspace.</p></div><button class="primary" id="new-group">＋ New group</button></div><div class="groups-list">${state.dashboard.groups.map((group)=>`<article class="group-card"><header><div><h3>${escapeHtml(group.name)}</h3><span class="subtle">${group.questions.length} question${group.questions.length === 1 ? "" : "s"} · ${group.scorerKind === "mean-v1" ? "Direction-aware mean" : "Custom JavaScript scorer"}${group.locked ? " · Locked after first run" : ""}</span></div><div class="row-actions"><button data-rename-group="${group.id}">Rename</button><button class="danger" data-delete-group="${group.id}">Delete</button></div></header>${group.description ? `<p>${escapeHtml(group.description)}</p>` : ""}<ol class="questions">${group.questions.map((question)=>`<li><span>${escapeHtml(question.text)} <span class="subtle">${question.key}</span></span><span class="direction ${question.direction}">${question.direction === "lower" ? "↓ lower is better" : "↑ higher is better"}</span></li>`).join("")}</ol></article>`).join("") || `<div class="empty-card">No groups yet.</div>`}</div></section>`;
